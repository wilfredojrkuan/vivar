...
// Global Variables
var isClosingCollapsibles = false;
var isJustPageLoaded = true;
...
theme.config = {
  ...
  mediaQuerySmall: 'screen and (max-width: 819px)',
  youTubeReady: false,
  vimeoReady: false,
  vimeoLoading: false,
...
if (console && console.log) {
  console.log('© 2022, ' +  json_shop_name + ' | All Rights Reserved');
}
...
(function(){
  ...
  theme.Variants = (function() {
    ...
    function Variants(options) {
      ...
      this.container.querySelectorAll(this.singleOptionSelector).forEach((el, index) => {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _getVariantFromOptions: function(lastSelectedOption) {
        ...
        return availableFullMatch || fullMatch || closestAvailableMatch || null;
      },
  ...
  theme.CartForm = (function() {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      onSubmit: function(evt) {
        setCookie(lopCookie, window.location.href, 24);  
      ...
      cartMarkup: function(html) {
        ...
        if (theme.motSettings.motCartUpsellShow) {
            // Check if there was added an upsell product to the cart
            //
            var cartItems = items.children;
            var upsellProduct;
            var upsellProducts = theme.motSettings.upsellProducts;
            var upsellItemAlreadyInCart = false;
      
            for (var i = 0; i < cartItems.length; i++) {
                var qty = 0;
                var price = 0.0;
                
                var key = cartItems[i].dataset.key;
                var variantId = parseInt(key.substring(0, key.indexOf(':')));
                var qtyEls = cartItems[i].getElementsByClassName("js-qty__num");
  
                if (theme.motSettings.motCartUpsellExcludeFromDiscounts) {
                    if (qtyEls.length > 0) {
                        qty = parseInt(qtyEls[0].value);
                    }
                
                    var priceStr = '';
                    var priceNormalEls = cartItems[i].getElementsByClassName("cart__price");
                    var priceSaleEls = cartItems[i].getElementsByClassName("sale-price");
          
                    if (priceSaleEls.length > 0) {
                        priceStr = priceSaleEls[0].innerText;  
                    } else if (priceNormalEls.length > 0) {
                        priceStr = priceNormalEls[0].innerText;
                    }
          
                    var priceArr = priceStr.match(/\d+(?:\,\d+)?/g);
                    price = parseFloat(priceArr[0].replace(',', '').replace('.', ''));
                }
      
                productLoop:
                for (var j = 0; j < upsellProducts.length; j++) {
                    var upsellVariants = upsellProducts[j].variants;
      
                    variantLoop:
                    for (var k = 0; k < upsellVariants.length; k++) {
                        var upsellVariant = upsellVariants[k];
                      
                        if (upsellVariant.id == variantId) {
                            if (qtyEls.length > 0) {
                                qtyEls[0].parentElement.querySelector('.js-qty__adjust.js-qty__adjust--plus').disabled = true;
                            }
      
                            upsellItemAlreadyInCart = true;
            ...
            if (upsellItemAlreadyInCart == false) {
                setCookie(upaCookie, -1, 0.000001);
            } else {
            ...
            if (parseInt(getCookie(upaCookie)) != 1 && upsellProducts) {
            	var rnp = 0;
    
            	if (upsellProducts.length > 0) {
            		rnp = parseInt(getCookie(rnpCookie));
            		
            		if (rnp < 0 || upsellProducts[rnp] == null || !upsellProducts[rnp].available) {
            			rnp = Math.floor(Math.random() * upsellProducts.length);
    
            			var reset = false;
            			
            			for (var i = rnp; i < upsellProducts.length; i++) {
                            rnp = i;
                            
                            if (upsellProducts[i].available == true) {
            					upsellProduct = upsellProducts[i];
            					break;
            				} else if (!reset && i + 1 >= upsellProducts.length) {
            					i = 0;
            					reset = true;
            				}
            			}
            			
            			if (upsellProduct && upsellProduct.available == true) {
                            setCookie(rnpCookie, rnp, 24);
            			}
            		} else {
                        upsellProduct = upsellProducts[rnp];
                    }
            	} else {
            		upsellProduct = upsellProducts[rnp];
            	}
            	...
            	if (upsellProduct && upsellProduct.available == true) {
            		var upsellVariant;
            		var upsellVariants = upsellProduct.variants;
            		
            		var rnv = 0;
                  
            		if (upsellVariants.length > 0) {
            			rnv = parseInt(getCookie(rnvCookie));
            			
            			if (rnv < 0 || upsellVariants[rnv] == null || !upsellVariants[rnv].available) {
                            rnv = Math.floor(Math.random() * upsellVariants.length);
    
                            var reset = false;
                            
                            for (var i = rnv; i < upsellVariants.length; i++) {
            					rnv = i;
            					
            					if (upsellVariants[i].available == true) {
            						upsellVariant = upsellVariants[i];
            						break;
            					} else if (!reset && i + 1 >= upsellVariants.length) {
            						i = 0;
            						reset = true;
            					}
            				}
            				
                            if (upsellVariant && upsellVariant.available == true) {
            					setCookie(rnvCookie, rnv, 24);
            				}
            			} else {
                			upsellVariant = upsellVariants[rnv];
                		}
            		} else {
            			upsellVariant = upsellVariants[rnv];
            		}
    
            		if (upsellVariant && upsellVariant.available == true && upsellProduct.title && upsellVariant.public_title && upsellVariant.price && upsellVariant.featured_image && upsellVariant.featured_image.src) {
                		var upsellImgSrc = upsellVariant.featured_image.src;
            			var upsellImgSrc_1 = upsellImgSrc.substring(0, upsellImgSrc.lastIndexOf('.'));
            			var upsellImgSrc_2 = upsellImgSrc.substring(upsellImgSrc.lastIndexOf('.'));
            			
            			var upsellImgSrc = upsellImgSrc_1 + '_{width}x' + upsellImgSrc_2;
            			
            			var price = `<span class="sale-price" style="font-weight: 400;">` + theme.Currency.formatMoney(upsellVariant.price, theme.settings.moneyFormat) + `</span>`;
            			
            			if (upsellVariant.compare_at_price != null && upsellVariant.compare_at_price > upsellVariant.price) {
                            price += `<span class="compare_at_price" style="margin-left: 10px; text-decoration: line-through;">` + theme.Currency.formatMoney(upsellVariant.compare_at_price, theme.settings.moneyFormat) + `</span>`;
            			}

                        var upsellHint = ``;
                      
                        if (theme.motSettings.motCartUpsellExcludeFromDiscounts) {
                            upsellHint = theme.motSettings.motCartUpsellHintText;
                        }

                        var prodMetafieldLinkToOriginalImageHtml = ``;
                        var prodMetafieldLinkToOriginalTitleHtml1 = ``;
                        var prodMetafieldLinkToOriginalTitleHtml2 = ``;

                        if (upsellMetaFields && upsellMetaFields[upsellProduct.handle] && upsellMetaFields[upsellProduct.handle].product_link_to_original.length > 0) {
                          prodMetafieldLinkToOriginalImageHtml = `href="` + upsellMetaFields[upsellProduct.handle].product_link_to_original + `" `;
                          prodMetafieldLinkToOriginalTitleHtml1 = `<a href="` + upsellMetaFields[upsellProduct.handle].product_link_to_original + `">`;
                          prodMetafieldLinkToOriginalTitleHtml2 = `</a>`;
                        }

            			var upsellProductHtml = `
                			<div id="motInCartUpsellEl" class="aos-init aos-animate upsellProductContainer" data-aos="">
                				<div class="upsellProductHeading">
                					` + theme.motSettings.motCartUpsellHeadingText + `
                				</div>
                				<div class="upsellProductSubRowContainer">
                					<div class="upsellProductImage">
                						<a ` + prodMetafieldLinkToOriginalImageHtml + `class="image-wrap" style="display: block; height: 100%;"><img class="lazyload" data-widths="[180, 360, 540]" data-aspectratio="" data-sizes="auto" alt="` + upsellProduct.title + `" data-srcset="` + upsellImgSrc + `">
                							<noscript>
                								<img class="lazyloaded" src="` + upsellImgSrc + `" alt="` + upsellProduct.title + `">
                							</noscript>
                						</a>
                					</div>
                					<div class="upsellProductSubColumnContainer" style="width: 100%; justify-content: space-between;">
                						<div class="upsellProductSubColumnContainer">
                							 ` + prodMetafieldLinkToOriginalTitleHtml1 + `<div class="cart__product-name upsellProductTitle">` + upsellProduct.title + `</div>` + prodMetafieldLinkToOriginalTitleHtml2 + `
                							<div class="upsellProductSubTitle">` + upsellVariant.public_title + `</div>
                                            <div class="upsellProductSubTitle">` + upsellHint + `</div>	
                						</div>
                						<div class="upsellProductSubRowContainer" style="justify-content: space-between; align-items: flex-end;">
                							<div class="cart__price" style="line-height: 1; width: 100%;">` + price + `</div>
                							<div style="position: relative; width: 46px; text-align: right; overflow: visible;">
                                                <button id="motUpsellATC" type="button" class="button_plus" onclick="
                                                    setCookie(upaCookie, 1, 24);
                                                    ...
                                                    fetch(theme.routes.cartAdd, {
                                                        method: 'POST',
                                                        body: JSON.stringify(motFormData),
                                                        credentials: 'same-origin',
                                                        headers: {
                                                            'Content-Type': 'application/json',
                                                            'X-Requested-With': 'XMLHttpRequest'
                        ...
                        this.products.insertAdjacentHTML('afterbegin', upsellProductHtml);
    
                        var motUpsellATC = document.getElementById('motUpsellATC');
    
                        setTimeout(function() {
                            if (!motUpsellATC) {
                                var motInCartUpsellEl = document.getElementById('motInCartUpsellEl');
      
                                if (motInCartUpsellEl) {
                                    motInCartUpsellEl.style.display = "none";
                                }
              				}
              			}, 100);
            		}
        ...
        // Regarding the use of free gifted products we need to check if only relevant products get counted for following discounts
        //
        var treeCounter = count;
        
        if (theme.motSettings.showPlantTrees && theme.motSettings.filterWordPlantTrees !== "") {
            treeCounter = 0;

            [...items.getElementsByClassName('cart__item')].forEach(el => {
            	if (el) {
                    if (el.innerHTML.toUpperCase().indexOf(theme.motSettings.filterWordPlantTrees.toUpperCase()) !== -1) {
                        var qtyEl = el.getElementsByClassName('js-qty__num');
      
                        if (qtyEl && qtyEl.length > 0) {
                            var actualCount = parseInt(qtyEl[0].value);
                            treeCounter += actualCount;
                        }
                    }
                }
        	});
        ...
        var importantProductsCounter = treeCounter - qtySum;

        // Update subtotal
        //
        var motSavingsDiv = document.getElementById("motSavingsDrawer");
        var motSumsDiv = document.getElementById("motSumsDrawer");
        ...
        if (motSumsDiv) {
          ...
          if (savings) {
            motSavings = parseInt(savings);
          }

          cartActionHTML += `<div class="cart__item-sub cart__item-row"><div class="cart__subtotal">Zwischensumme</div><div class="data-subtotal">` + theme.Currency.formatMoney(parseInt(subtotal) + motSavings, theme.settings.moneyFormat) + `</div></div>`;

          var bundleSavingMot = 0.0;
          var bundleDiscountMessage = "";
          var bundleDiscountMessageProgressBar = "";

          if (motSavingsDiv) {
            if (theme.motSettings.applyBundleDiscounts === true) {
              bundleDiscountMessageProgressBar += `<table class="table-no-border-mot4" style="background-color: rgba(0, 0, 0, 0.0) !important; border-spacing: 0 !important; width: 100%; text-align: center; margin-top: 10px;"><tr>`;
  
              if (importantProductsCounter === 1) {
                bundleDiscountMessage += theme.motSettings.cartUpsellTitle1 + `<div style="padding-top: 3px; font-size: 15px; font-weight: 300 !important; letter-spacing: 0 !important;">` + theme.motSettings.cartUpsellLabel1 + `</div>`;
  
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-blue w3-round-large">0%</div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-light-grey w3-round-large">` + theme.motSettings.discountValue1 + `%</div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-light-grey w3-round-large">` + theme.motSettings.discountValue2 + `%</div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-light-grey w3-round-large">` + theme.motSettings.discountValue3 + `%</div></td>`;
              } else if (importantProductsCounter === 2) {
                bundleDiscountMessage += theme.motSettings.cartUpsellTitle2 + `<div style="padding-top: 3px; font-size: 15px; font-weight: 300 !important; letter-spacing: 0 !important;">` + theme.motSettings.cartUpsellLabel2 + `</div>`;
  
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-blue w3-round-large"></div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-blue w3-round-large">` + theme.motSettings.discountValue1 + `%</div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-light-grey w3-round-large">` + theme.motSettings.discountValue2 + `%</div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-light-grey w3-round-large">` + theme.motSettings.discountValue3 + `%</div></td>`;
              } else if (importantProductsCounter === 3) {
                bundleDiscountMessage += theme.motSettings.cartUpsellTitle3 + `<div style="padding-top: 3px; font-size: 15px; font-weight: 300 !important; letter-spacing: 0 !important;">` + theme.motSettings.cartUpsellLabel3 + `</div>`;
  
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-blue w3-round-large"></div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-blue w3-round-large"></div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-blue w3-round-large">` + theme.motSettings.discountValue2 + `%</div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-light-grey w3-round-large">` + theme.motSettings.discountValue3 + `%</div></td>`;
              } else if (importantProductsCounter >= 4) {
                bundleDiscountMessage += theme.motSettings.cartUpsellTitle4 + `<div style="padding-top: 3px; font-size: 15px; font-weight: 300 !important; letter-spacing: 0 !important;">` + theme.motSettings.cartUpsellLabel4 + `</div>`;
                ...
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-blue w3-round-large"></div></td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 1%; line-height: 0;">&nbsp;</td>`;
                bundleDiscountMessageProgressBar += `<td style="width: 24%"><div class="w3-container w3-blue w3-round-large">` + theme.motSettings.discountValue3 + `%</div></td>`;
              }
  
              bundleDiscountMessageProgressBar += `</tr></table>`;
              bundleDiscountMessage += bundleDiscountMessageProgressBar;
              bundleDiscountMessage += `</div></div>`;
            } else if (theme.motSettings.applyBundleDiscounts === false && theme.motSettings.cartNoneUpsellTitle && theme.motSettings.cartNoneUpsellLabel) {
              if (count > 0) {
                bundleDiscountMessage = `<span style="color:` + theme.motSettings.cartNoneUpsellTitleColor + `;">` + theme.motSettings.cartNoneUpsellTitle + `</span><div style="padding-top: 3px; font-size: 15px; font-weight: 300 !important; letter-spacing: 0 !important;">` + theme.motSettings.cartNoneUpsellLabel + `</div>`;
              } else {
                bundleDiscountMessage = ``;
              }
            }

            motSavingsDiv.innerHTML = bundleDiscountMessage;
          }

          if (motSavings > 0) {
            var discPerc = Math.round((1.0 - (parseInt(subtotal) / (parseInt(subtotal) + motSavings))) * 100.0);

            cartActionHTML += `<div class="cart__item-sub cart__item-row"><div class="cart__subtotal">Du sparst (` + discPerc + `%)</div><div class="data-subtotal">-` + theme.Currency.formatMoney(motSavings, theme.settings.moneyFormat) + `</div></div>`;
          }

          if (theme.motSettings.shippingFreeShipping === true) {
            cartActionHTML += `<div class="cart__item-sub cart__item-row"><div class="cart__subtotal">Versand</div><div class="data-subtotal">Kostenlos</div></div>`;
          }
       
          cartActionHTML += `<div style="border-top: 1px dashed #BFBDBD; margin-top: 10px; padding-bottom: 10px;"></div>`;
          cartActionHTML += `<div class="cart__item-sub cart__item-row"><div class="cart__subtotal" style="font-weight: normal !important;">Gesamtsumme</div><div class="data-subtotal" style="font-weight: normal !important;">` + theme.Currency.formatMoney(subtotal, theme.settings.moneyFormat) + `</div></div>`;
          
          if (theme.motSettings.showPlantTrees === true) {
              var treeFill = treeCounter + ` `;
            ...
            cartActionHTML += `<div style="margin-top: 12px;"><table class="table-no-border-mot4" style="background-color: rgba(0, 0, 0, 0.0) !important; border-spacing: 0 !important; width: auto; margin-left: auto; margin-right: auto;"><tr><td style="height: 21px; width: 21px;"><img class="lazyload" src="https://cdn.shopify.com/s/files/1/0599/7648/2966/files/PlantTreeIcon3.svg?v=1645273543" /></td><td style="padding-left: 8px !important; padding-top: 2px !important; font-size: 14px; font-weight: 300;">Mit deinem Kauf pflanzt du ` + treeFill + `</td></tr></table></div>`;
          } else {
            cartActionHTML += `<div style="margin-top: 22px;"></div>`
          }

          motSumsDiv.innerHTML = cartActionHTML;
        }
        ...
        if (bubbles.length) {
          if (count > 0) {
            bubbles.forEach(b => {
              ...
              if (theme.motSettings.cartIconEnableCount === true) {
                b.innerHTML = `<div>` + count + `</div>`;

                if (count > 9) {
                  b.classList.add('motCartLinkBubbleBig');
                } else {
                  b.classList.remove('motCartLinkBubbleBig');
                }
              }
            });
          } else {
  ...
  theme.collapsibles = (function() {
    ...
    function closeOpenCollapsibles(arrCollapsibleTriggers, isProd = false, isAbove = true) {
  	  isClosingCollapsibles = true;
      
      for (var j = 0; j < arrCollapsibleTriggers.length; j++) {
        if (isProd && isAbove == false && arrCollapsibleTriggers[j].getBoundingClientRect()) {
          window.scrollBy({
              top: arrCollapsibleTriggers[j].getBoundingClientRect().top + 7
          });
        }
        
        arrCollapsibleTriggers[j].click();
      }
            
      isClosingCollapsibles = false;
    }
    
    function scrollToClickedCollapsible(element, timeout) {
      setTimeout(function() {
        const announcementEl = document.getElementById("hura-announcement-bar-id");
        const headerEl = document.getElementById("SiteHeader");

        if (headerEl) {
          var motHeightAnnouncementBar = 0;
          var motIsStickyHeader = '';

          const motHeightHeader = headerEl.clientHeight;

          if (announcementEl) {
            motHeightAnnouncementBar = announcementEl.clientHeight;
          }

          if (headerEl.dataset) {
            motIsStickyHeader = headerEl.dataset.sticky;  
          }

          if (motHeightHeader && motHeightHeader > 0) {
            if (motIsStickyHeader === 'true') {
              if (motHeightAnnouncementBar && motHeightAnnouncementBar > 0) {
                window.scrollBy({
                  top: element.getBoundingClientRect().top - motHeightAnnouncementBar - motHeightHeader + 25,
                  behavior: "smooth"
                });
              } else {
                window.scrollBy({
                  top: element.getBoundingClientRect().top - motHeightHeader,
                  behavior: "smooth"
                });
              }
            } else {
              window.scrollBy({
                top: element.getBoundingClientRect().top + 7,
                behavior: "smooth"
              });
            }
          }
    ...
    function toggle(evt) {
      ...
      if (theme.motSettings.openFirstCollapsible === false) {
      	isJustPageLoaded = false;
      }
      
      var clickedOnProductTrigger = el.getAttribute('aria-controls').includes('Product');
      var clickedOnFaqTrigger = el.getAttribute('aria-controls').includes('FAQ');
      var clickedOnFooterTrigger = el.getAttribute('aria-controls').includes('Footer');
      
      if (isClosingCollapsibles === false && theme.motSettings.closeOpenCollapsibles === true) {
        var arrProduct = [];
        var arrFaq = [];
        var arrFooter = [];
        
        var isNewClickedCollapsibleAbove = true;
        ...
        for (var i = 0; i < elementsCollapsibleTriggers.length; i++) {
          ...
          if (clickedOnProductTrigger && elementsCollapsibleTriggers.item(i) === el && arrProduct.length > 0 && !elementsCollapsibleTriggers.item(i).classList.contains(classes.open)) {
            isNewClickedCollapsibleAbove = false;
          }

          if (elementsCollapsibleTriggers.item(i) !== el && elementsCollapsibleTriggers.item(i).classList.contains(classes.open)) {
            if (elActualAriaControl.includes('Product')) {
              arrProduct.push(elementsCollapsibleTriggers.item(i));
            } else if (elActualAriaControl.includes('FAQ')) {
              arrFaq.push(elementsCollapsibleTriggers.item(i));
            } else if (elActualAriaControl.includes('Footer')) {
              arrFooter.push(elementsCollapsibleTriggers.item(i));
            }
          }
        }
        
        if (clickedOnProductTrigger && arrProduct.length > 0) {
          closeOpenCollapsibles(arrProduct, true, isNewClickedCollapsibleAbove);  
        }

        if (clickedOnFaqTrigger && theme.motSettings.sameCollapsibleSettingsOnFaq === true && arrFaq.length > 0) {
          closeOpenCollapsibles(arrFaq);  
        }

        if (clickedOnFooterTrigger && theme.motSettings.sameCollapsibleSettingsOnFooter === true && arrFooter.length > 0) {
          closeOpenCollapsibles(arrFooter);  
        }
      }
      ...
      if (isClosingCollapsibles === false) {
        if (isOpen === false && theme.motSettings.scrollClickedCollapsible === true && ((clickedOnProductTrigger === true && isJustPageLoaded === false) || (clickedOnFaqTrigger === true && theme.motSettings.sameCollapsibleSettingsOnFaq === true) || (clickedOnFooterTrigger === true && theme.motSettings.sameCollapsibleSettingsOnFooter === true))) {
          scrollToClickedCollapsible(el, 400);
        }
      }
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      addItemFromForm: function(evt, callback){
        ...
        .then(function(data) {
          ...
          var motAtcBtnModal = document.getElementById("mot-modal");

          var motShowSticky = true;

          if (json_template === 'index' && theme.motSettings.fullStickyHideIndex) {
            motShowSticky = false;
          }
          ...
          if (data.status === 422) {
            ...
            if (motShowSticky && theme.motSettings.stickyAtcType == 'new_style' && theme.motSettings.fullStickyStayOpened && motAtcBtnModal && motAtcBtnModal.classList.contains('active')) {
              var motATCBtnTextNormal = document.getElementById('motATCBtnTextNormal');
              var motATCBtnText = document.getElementById('motATCBtnText');
        
      		if (motATCBtnTextNormal && motATCBtnText) {
                motATCBtnTextNormal.style.display = "none";
                motATCBtnText.style.display = "block";
    
                setTimeout(function() {
                    motATCBtnTextNormal.style.display = "block";
                    motATCBtnText.style.display = "none";
                    motATCBtnText.innerHTML = 'Nicht mehr verfügbar';
                }, 3000);
              }
            }
          } else {
      ...
      init: function() {
        this.plus.addEventListener('click', function() {
          if (!this.plus.disabled) {
            ...
            var motQtyPlusId = 'motQtyPlus';
            
            document.dispatchEvent(new CustomEvent('motQty:quantity', {
                detail: [this.plus.id == motQtyPlusId ? true : false]
            }));
            
            if (this.plus.id != motQtyPlusId && theme.motSettings.showBundleDiscounts === true) {
              if (typeof motFunc === 'function') {
      		  	motFunc(qty + 1);
              }
            }
          }
        ...
        this.minus.addEventListener('click', function() {
          ...
          var motQtyMinusId = 'motQtyMinus';
          
          document.dispatchEvent(new CustomEvent('motQty:quantity', {
              detail: [this.minus.id == motQtyMinusId ? true : false]
          }));
          
          if (this.minus.id != motQtyMinusId && qty > 1 && theme.motSettings.showBundleDiscounts === true) {
            if (typeof motFunc === 'function') {
    		  	motFunc(qty - 1);
            }
          }
  ...
  theme.Slideshow = (function() {
    ...
    slideshow.prototype = Object.assign({}, slideshow.prototype, {
      ...
      slideChange: function(index) {
        ...
        var variantImageShow = document.getElementById('imgVariantShow');

        if (variantImageShow) {
          var hideVariantLabel = true;
          
          if (theme.motSettings.productImageShowVariantLabel && this.el) {
            var slideWrapper = this.el.getElementsByClassName('is-selected');
  
            if (slideWrapper && slideWrapper.length > 0) {
              var slideImg = slideWrapper[0].getElementsByTagName('img');
  
              if (slideImg && slideImg.length > 0) {
                var dataImgId = slideImg[0].getAttribute('data-image-id');
  
                if (dataImgId && dataImgId !== undefined && json_product.variants.length > 1) {
                  for (var i = 0; i < json_product.variants.length; i++) {
                    var currentVar = json_product.variants[i];
                    
                    if (currentVar && currentVar !== undefined && parseInt(currentVar.featured_media.id) == parseInt(dataImgId)) {
                      var variantLabelName = document.getElementsByClassName('variant__label');
  
                      if (variantLabelName && variantLabelName.length > 0 && currentVar.option1) {
                          // variantImageShow.innerHTML = variantLabelName[0].textContent + ':&nbsp;' + currentVar.title;
                          variantImageShow.innerHTML = variantLabelName[0].textContent.trim() + ':&nbsp;' + currentVar.option1.trim();
  ...
  theme.VariantAvailability = (function() {
    ...
    availability.prototype = Object.assign({}, availability.prototype, {
      ...
      manageOptionState: function(option, values) {
        ...
        this.container.querySelectorAll('.variant-input-wrap[data-index="'+ option +'"]').forEach(group => {
    ...
    CartDrawer.prototype = Object.assign({}, CartDrawer.prototype, {
      init: function() {
        ...
        document.addEventListener('ajaxProduct:added', function(evt) {
          ...
          if (theme.motSettings.stickyAtcType != 'new_style' || !motShowSticky) {
            this.open();
          }
        }.bind(this));
  ...
  theme.headerNav = (function() {
    ...
    function stickyHeader() {
      ...
      window.on('scroll' + config.namespace, theme.utils.throttle(10, stickyHeaderScroll));
    ...
    function stickyHeaderHeight() {
      ...
      if (headerEl && headerEl.dataset) {
        var motIsStickyHeader = headerEl.dataset.sticky;

        if (motIsStickyHeader === 'true') {
          var motProdSingleSticky = document.getElementById('motProductSingleSticky');

          if (motProdSingleSticky) {
            var motAnnouncementBar = document.getElementById("hura-announcement-bar-id");
            
            if (motAnnouncementBar) {
              var motHeightAnnouncementBar = motAnnouncementBar.clientHeight;

              if (motHeightAnnouncementBar && h) {
                  motProdSingleSticky.style.top = h + motHeightAnnouncementBar + 'px';
              } else {
                  motProdSingleSticky.style.top = 118 + 'px';
              }
            } else {
              if (h) {
                motProdSingleSticky.style.top = 28 + h + 'px';
              } else {
                motProdSingleSticky.style.top = 118 + 'px';
              }
            }
    ...
    function scrollHandler() {
      if (window.scrollY > 50) { // original 250
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      updatePrice: function(evt) {
        ...
        if (variant) {
          ...
          var motVariantPrice0 = document.getElementById("variantPrice0");
          var motVariantPrice0_ = document.getElementById("variantPrice0_");
          var motVariantPrice0__ = document.getElementById("variantPrice0__");
          var motVariantPrice1 = document.getElementById("variantPrice1");
          var motVariantPrice1_ = document.getElementById("variantPrice1_");
          var motVariantPrice1__ = document.getElementById("variantPrice1__");
          var motVariantPrice2 = document.getElementById("variantPrice2");
          var motVariantPrice2_ = document.getElementById("variantPrice2_");
          var motVariantPrice2__ = document.getElementById("variantPrice2__");
          var motVariantPrice3 = document.getElementById("variantPrice3");
          var motVariantPrice3_ = document.getElementById("variantPrice3_");
          var motVariantPrice3__ = document.getElementById("variantPrice3__");

          if (motVariantPrice1__ && motVariantPrice2__ && motVariantPrice3__) {
          	if (theme.motSettings.applySummarizedPrice === true) {
			  motVariantPrice0__.classList.add('hideVariantPricePiece');
              motVariantPrice1__.classList.add('hideVariantPricePiece');
              motVariantPrice2__.classList.add('hideVariantPricePiece');
              motVariantPrice3__.classList.add('hideVariantPricePiece');
              
              motVariantPrice0__.classList.remove('adjustVariantPricePiece');
              motVariantPrice1__.classList.remove('adjustVariantPricePiece');
              motVariantPrice2__.classList.remove('adjustVariantPricePiece');
              motVariantPrice3__.classList.remove('adjustVariantPricePiece');
            } else {
              motVariantPrice0__.classList.remove('hideVariantPricePiece');
              motVariantPrice1__.classList.remove('hideVariantPricePiece');
              motVariantPrice2__.classList.remove('hideVariantPricePiece');
              motVariantPrice3__.classList.remove('hideVariantPricePiece');
              
              motVariantPrice0__.classList.add('adjustVariantPricePiece');
              motVariantPrice1__.classList.add('adjustVariantPricePiece');
              motVariantPrice2__.classList.add('adjustVariantPricePiece');
              motVariantPrice3__.classList.add('adjustVariantPricePiece');
            }
          }
          
          var showMotSavings = theme.motSettings.showYouSave;
          var motTaxesShippingDiv = document.getElementById("mot-satc-taxes-shipping");
          var motSavingsTextDiv = document.getElementById("motChangeSavingsJS");
		  var motSavingsText = ``;
            
          if (showMotSavings === true && json_product && json_product !== undefined && json_product.tags && json_product.tags !== undefined) {
            Array.from(json_product.tags).every((prodTag) => {
              if (prodTag === 'DONTSHOWSAVING') {
                showMotSavings = false;
              }
            });
          } else if (motSavingsTextDiv == null || motSavingsTextDiv == undefined) {
            showMotSavings = false;
          }
          ...
          if (variant.compare_at_price > variant.price) {
            ...
            if (showMotSavings === true) {
              var diffSaving = variant.compare_at_price - variant.price;
              var diffSavingPercentage = Math.ceil((1 - (variant.price / variant.compare_at_price)) * 100.0);
              
              motSavingsTextDiv.style.display = "block";

              if (motTaxesShippingDiv) {
                motTaxesShippingDiv.style.marginTop = "10px";
              }
              
			  motSavingsText = `DU SPARST ` + diffSavingPercentage + `% (` + theme.Currency.formatMoney(diffSaving, theme.settings.moneyFormat) + `)`;
            }
            
            if (motVariantPrice0 && motVariantPrice0_ && motVariantPrice1 && motVariantPrice1_ && motVariantPrice2 && motVariantPrice2_ && motVariantPrice3 && motVariantPrice3_) {
              motVariantPrice0.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
              motVariantPrice0_.innerHTML = theme.Currency.formatMoney(variant.compare_at_price, theme.settings.moneyFormat);
              
              if (theme.motSettings.applySummarizedPrice === true) {
                motVariantPrice1.innerHTML = theme.Currency.formatMoney((variant.price * 2) - Math.floor(variant.price * 2 * (theme.motSettings.discountValue1 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice1_.innerHTML = theme.Currency.formatMoney(Math.floor(variant.price * 2), theme.settings.moneyFormat);
                motVariantPrice2.innerHTML = theme.Currency.formatMoney((variant.price * 3) - Math.floor(variant.price * 3 * (theme.motSettings.discountValue2 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice2_.innerHTML = theme.Currency.formatMoney(Math.floor(variant.price * 3), theme.settings.moneyFormat);
                motVariantPrice3.innerHTML = theme.Currency.formatMoney((variant.price * 4) - Math.floor(variant.price * 4 * (theme.motSettings.discountValue3 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice3_.innerHTML = theme.Currency.formatMoney(Math.floor(variant.price * 4), theme.settings.moneyFormat);
              } else {
                motVariantPrice1.innerHTML = theme.Currency.formatMoney(Math.ceil(variant.price * (1.0 - theme.motSettings.discountValue1 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice1_.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
                motVariantPrice2.innerHTML = theme.Currency.formatMoney(Math.ceil(variant.price * (1.0 - theme.motSettings.discountValue2 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice2_.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
                motVariantPrice3.innerHTML = theme.Currency.formatMoney(Math.ceil(variant.price * (1.0 - theme.motSettings.discountValue3 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice3_.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
              }
            }
		  } else {
            if (showMotSavings === true) {
              motSavingsTextDiv.style.display = "none";
              
              if (motTaxesShippingDiv) {
                motTaxesShippingDiv.style.marginTop = "5px";
              }
            }
            
            if (motVariantPrice0 && motVariantPrice0_ && motVariantPrice1 && motVariantPrice1_ && motVariantPrice2 && motVariantPrice2_ && motVariantPrice3 && motVariantPrice3_) {
              motVariantPrice0.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
              motVariantPrice0_.innerHTML = '';
              
              if (theme.motSettings.applySummarizedPrice === true) {
                motVariantPrice1.innerHTML = theme.Currency.formatMoney((variant.price * 2) - Math.floor(variant.price * 2 * (theme.motSettings.discountValue1 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice1_.innerHTML = theme.Currency.formatMoney(variant.price * 2, theme.settings.moneyFormat);
                motVariantPrice2.innerHTML = theme.Currency.formatMoney((variant.price * 3) - Math.floor(variant.price * 3 * (theme.motSettings.discountValue2 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice2_.innerHTML = theme.Currency.formatMoney(variant.price * 3, theme.settings.moneyFormat);
                motVariantPrice3.innerHTML = theme.Currency.formatMoney((variant.price * 4) - Math.floor(variant.price * 4 * (theme.motSettings.discountValue3 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice3_.innerHTML = theme.Currency.formatMoney(variant.price * 4, theme.settings.moneyFormat);
              } else {
                motVariantPrice1.innerHTML = theme.Currency.formatMoney(Math.floor(variant.price * (1.0 - theme.motSettings.discountValue1 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice1_.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
                motVariantPrice2.innerHTML = theme.Currency.formatMoney(Math.floor(variant.price * (1.0 - theme.motSettings.discountValue2 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice2_.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
                motVariantPrice3.innerHTML = theme.Currency.formatMoney(Math.floor(variant.price * (1.0 - theme.motSettings.discountValue3 / 100.0)), theme.settings.moneyFormat);
                motVariantPrice3_.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
              }
            }
          ...
          if (showMotSavings === true) {
            motSavingsTextDiv.innerHTML = motSavingsText;
          }
        }
      },
      ...
      updateInventory: function(evt) {
        ...
        if (variant.inventory_management === 'shopify' && window.inventories && window.inventories[this.productId]) {
          ...
          // Re-formatting of inventory status sales point
          //
          var motInv = document.getElementById('mot-inventory');

          if (motInv) {
            var motInvStr = motInv.textContent;  

            if (motInvStr) {
              const motContentArr = motInvStr.split(" | ");

              if (motContentArr.length > 1) {
                var outputStr = motContentArr[0] + " <span style=\"font-weight:300;\">| " + motContentArr[motContentArr.length - 1] + "</span>";

                motInv.innerHTML = outputStr;
              }
            }
          }
        }
      ...
      toggleInventoryQuantity: function(variant, show, qty) {
        ...
        if (parseInt(qty) <= parseInt(this.settings.inventoryThreshold) && parseInt(qty) > 0) {
          if (!el.parentNode.classList.contains('inventory--low')) {el.parentNode.classList.add('inventory--low');}
          if (el.parentNode.classList.contains('inventory--no')) {el.parentNode.classList.remove('inventory--no');}
          el.textContent = theme.strings.stockLowLabel.replace('[count]', qty);
        } else if (parseInt(qty) > parseInt(this.settings.inventoryThreshold)) {
          if (el.parentNode.classList.contains('inventory--low')) {el.parentNode.classList.remove('inventory--low');}
          if (el.parentNode.classList.contains('inventory--no')) {el.parentNode.classList.remove('inventory--no');}
          el.textContent = theme.strings.inStockLabel;
        } else if (parseInt(qty) == 0) {
          if (el.parentNode.classList.contains('inventory--low')) {el.parentNode.classList.remove('inventory--low');}
          if (!el.parentNode.classList.contains('inventory--no')) {el.parentNode.classList.add('inventory--no');}
          el.textContent = theme.strings.soldOut;          
        }
      },
